...
export function VideoPlayer({ contentId }: { contentId: string }) {
  ...
  useEffect(()=>{load()},[]);
  return <div className="panel">{loading&&<p>Menyiapkan stream…</p>}{error?<><p>{error}</p><button className="button" onClick={load}>Coba lagi</button></>:<video ref={ref} controls playsInline style={{width:"100%",maxHeight:"75vh",background:"#000"}}/>}</div>
}
