...
export default async function Admin({params}:{params:Promise<{section:string[]}>}){
  ...
  return <main className="admin-context"><AdminLayout role={user.role} title={title}>
    {section==="api-clipku"?<AdminIntegration/>:section==="error-logs"?<AdminMonitoring/>:["settings","payment-settings"].includes(section)?<AdminSettingsForm section={section}/>:<AdminConsole section={section}/>}
  </AdminLayout></main>
}
