...
const input=z.object({id:z.string().optional(),name:z.string().trim().min(2).max(80),slug:z.string().regex(/^[a-z0-9-]+$/),price:z.number().min(0),durationDays:z.number().int().min(1).max(3650),maxDevices:z.number().int().min(1).max(20),maxResolution:z.enum(["480p","720p","1080p","4K"]),isTrial:z.boolean(),isActive:z.boolean()});
